...
const Staff = ({ data }) => {
    ...
    return <ContextMenuTrigger id={isSelf ? STAFF_CONTEXTMENU_ID : OTHER_STAFF_CONTEXTMENU_ID} attributes={{ 'staffjson': JSON.stringify(data) }}>
...
const StaffGroup = ({ staffs, isExpand, panelId, expandHandle }) => {
    ...
    return <StaffGroupDiv key={groupName}>
        {groupName &&
...
export const OtherStaffContextMenu = ({ contextMenuId, dispatch }) => {
    ...
    return <ContextMenu id={contextMenuId}>
...
const Loading = withLoading(LoadingDiv, '客服列表')
    ...
    render() {
        ...
        return <div>
            <ExpandPanel title={'部门-组别'} count={staffCount} isExpand={this.getExpandState('staffRoot')} expandHandle={this.handleExpandState} panelId={'staffRoot'}>
                {!staffs && <Loading />}
                {noGroupStaffs && <GroupUl> {noGroupStaffs.map(staff => <Staff data={staff} key={staff.StaffId} />)} </GroupUl>}
                {groups && <GroupUl>
